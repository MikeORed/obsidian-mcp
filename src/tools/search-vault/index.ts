...
import { safeJoinPath, normalizePath } from "../../utils/path.js";
...
const schema = z
  .object({
    ...
    query: z
      ...
      .describe(
        "Search query (required). For text search use the term directly, for tag search use tag: prefix"
      ),
    path: z
      .string()
      .optional()
...
// Extended search options with pagination
interface ExtendedSearchOptions extends SearchOptions {
  page?: number;
  pageSize?: number;
}

// Extended search operation result with pagination
interface PaginatedSearchOperationResult extends SearchOperationResult {
...
): Promise<PaginatedSearchOperationResult> {
  try {
...
Note: To get a list of available vaults, use the list-vaults prompt instead of this search tool.`,
      ...
      handler: async (args, vaultPath, _vaultName) => {
        ...
        let formattedResult = formatSearchResult(result);
        ...
        return createToolResponse(formattedResult);
      },
    },
    vaults
  );
